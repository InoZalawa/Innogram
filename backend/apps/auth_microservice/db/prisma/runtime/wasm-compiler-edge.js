...
var Ye,
  ...
  Ki = fe(() => {
    ...
    S.times = S.mul = function (e) {
      ...
      for (; !o[--s]; ) o.pop();
      return (
        t ? ++r : o.shift(), (e.d = o), (e.e = r), q ? N(e, A.precision) : e
      );
    };
...
function Gu(e, t) {
  ...
  if (a) {
    ...
    if (e.dependentArgumentPath) {
      ...
      t.addErrorMessage(
        (f) =>
          `Argument \`${f.green(o)}\` is required because argument \`${f.green(
            m
          )}\` was provided.`
      );
    }
...
function zn(e) {
  throw Qr(e)
    ? new te(
        ...
        }\`. Message: \`${e.cause.originalMessage ?? Fs(e)}\``,
        'P2010',
  ...
  return await t.runInChildSpan(
    ...
    async () => {
      let o = new Date(),
        ...
        m = w.now();
      return (
        n?.({ timestamp: o, duration: m - s, query: e.sql, params: e.args }), a
      );
    }
  );
}
  ...
  switch (n) {
    ...
    case 'parameterTuple':
      return `(${
        e.value.length == 0
          ? 'NULL'
          : e.value.map(() => Ti(t, r.placeholderNumber++)).join(',')
      })`;
